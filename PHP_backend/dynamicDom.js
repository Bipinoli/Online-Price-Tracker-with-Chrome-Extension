...
function createListItem(data) {
    ...
    for (let j=0; j<desendents.length; j++) {
        ...
        if (child.className && child.className == "detail-price1") {
            let txt = document.createTextNode("Current Price ");
            child.prepend(txt);
        }
        if (child.className && child.className == "detail-price2") {
            let txt = document.createTextNode("Desired Price ");
            child.prepend(txt);
        }
...
function addListItem(data) {
    document.getElementsByClassName("list-view")[0].appendChild(createListItem(data));
